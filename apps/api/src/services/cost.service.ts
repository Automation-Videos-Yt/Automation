...
import {
  findRunId,
  getRunCostInputs,
  hasSuccessfulScriptLog,
  listCostHistoryLogs,
  listHistoricalSignalRows,
  type CostRunProfile,
...
import {
  ...
  computeVoiceUsd,
  computeWhisperUsd,
  deriveThumbnailUsdFromLog,
  deriveVoiceCostFromAgentLog,
  THUMBNAIL_PER_IMAGE,
  voiceProviderFromId,
} from "../utils/cost-calculation.utils";
...
async function loadHistoricalSignals(
  ...
  const { topicMemoryRows, hookMemoryRows, recentRuns } =
    await listHistoricalSignalRows(niche);
...
async function runExists(runId: string): Promise<boolean> {
  const run = await findRunId(runId);
  return !!run;
}
...
export async function estimateRunCost(
  ...
  const { voice, thumbnailLog, voiceLog, runProfile } =
    await getRunCostInputs(runId);
  ...
  const llmHit = await hasSuccessfulScriptLog(runId);
...
export async function getRunCostHistory(
  ...
  const logs = await listCostHistoryLogs(runId);
